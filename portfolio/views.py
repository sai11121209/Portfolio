...
def Portfolio(request):
    ...
    with open("requirements.txt", encoding="utf-16") as f:
        ...
        requirements = [
            url[0]
            + requirement[0 : requirement.find("==")]
            + url[1]
            + requirement[0 : requirement.find("==")]
            for requirement in f.readlines()
        ]
    with open("templates/portfolio/tdrapp_requirements.txt", encoding="utf-16") as f:
        url = ["https://img.shields.io/pypi/v/", "?label="]
        tdrapp_requirements = [
            url[0]
            + requirement[0 : requirement.find("==")]
    ...
    para = {
        ...
        "tdrapp_requirements": tdrapp_requirements,
    }
...
def ContactDetail(request, pk, username):
    if get_object_or_404(Contact, pk=pk).name == str(request.user):
        ...
        return render(
            request,
            "portfolio/contactdetail.html",
            {"username": request.user, "contact": contact},
        )
    return redirect("portfolio:home")
...
def ContactEdit(request, pk, username):
    if get_object_or_404(Contact, pk=pk).name == str(request.user):
        ...
        return render(
            request,
            "portfolio/contactedit.html",
            {"username": request.user, "contact": contact, "form": form},
        )
    return redirect("portfolio:home")
